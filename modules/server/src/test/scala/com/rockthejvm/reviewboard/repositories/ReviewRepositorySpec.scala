...
object ReviewRepositorySpec extends ZIOSpecDefault with RepositorySpec:
  ...
  private val goodReview = Review(1L, 1L, 1L, 5, 5, 5, 5, 10, "all good", Instant.now(), Instant.now())
  private val badReview = Review(2L, 1L, 1L, 1, 1, 1, 1, 1, "all bad", Instant.now(), Instant.now())
