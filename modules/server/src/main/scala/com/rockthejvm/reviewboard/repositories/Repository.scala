 ...
 * @tparam A the domain/model type representing rows in the db table
 */
trait Repository[A]:
  /**
   * Insert an item in the table
   * @param item the item to insert
   * @return a task containing the inserted item
   */
   ...
   * @param op the transformer function to invoke on the existing item
   * @return a task containing the result of invoking op on the item
   */
   ...
   * @return a task containing the deleted item
   */
   ...
   * @return a task containing an option of the found item
   */
