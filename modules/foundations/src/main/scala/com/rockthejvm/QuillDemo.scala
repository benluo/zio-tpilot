...
class JobRepositoryLive(quill: Quill.Postgres[SnakeCase]) extends JobRepository:
  ...
  inline given insMeta: InsertMeta[Job] = insertMeta[Job](_.id) // columns to be excluded in inserts
  inline given upMeta: UpdateMeta[Job]  = updateMeta[Job](_.id) // columns to be excluded in updates
